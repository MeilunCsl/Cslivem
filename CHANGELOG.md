...
| v0.9.0 | Polish | 性能优化、UI 细节、动画 | ✅ 已完成 |
| v1.0.0 | Release | 正式发布版 | ✅ 已完成 |
...
### 变更
- 搜索页入场动画（fade-up 系统 + ready 状态）
- 工具页接入真实 tool-registry 数据（不再硬编码）
- 工具页即将推出列表扩展至 6 个
- 所有页面入场动画统一（fade-up + delay 系统）

---


## v1.6.0 — AI-UX（2026-06-14）
...
### ??
- `pages/home/home.js` ? ????? bindinput + bindconfirm???? AI chat
- `pages/home/home.wxml` ? ?? input ??????
- `pages/ai-chat/ai-chat.js` ? ?? query ????? buildContext?????
- `pages/ai-chat/ai-chat.wxml` ? ??????????????
- `pages/ai-chat/ai-chat.wxss` ? ?????47 ??????
- `pages/graph-view/graph-view.js` ? ????????
- `pages/graph-view/graph-view.wxml` ? ???? chip ??
- `pages/graph-view/graph-view.wxss` ? ?? chip ??
...
## v1.5.0 — ECS-Sync（2026-06-14）

### ??
- ECS ????? `miniprogram/ecs-adapter.js` ?????
  - ?? HTTP ???wx.request?OpenAI ?? REST ???
  - ?????packageLocalData???? + ?? + ?? + ??
  - ?????importDataToStore???????ID ?? + updatedAt ???
  - ?????last-write-wins ??
  - ???????pending/done/failed ????
  - ??????? 5 ??
- ???? `pages/settings/`?
  - AI Provider ???MiMo / ECS?
  - MiMo ???endpoint?apiKey?model
  - ECS ???endpoint?apiKey?model
  - ??????
  - ??????????/??/???
  - ???????/?/??/????
  - ?????JSON ???????
...
### ????
- ECS ?????OpenAI ???Bearer Token + JSON?
- ???????? ID ???updatedAt ?????
- ????????? adjacency index
- ???????????graph + notes + calendar + ledger?
